...
- None (no legacy files existed)

If these reappear after a sync, delete them again. All configuration is in `pyproject.toml`.

### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `basana/` -> `src/basana/`
- **Import unchanged**: `import basana` still works

If upstream adds files to the old location, move them to `src/basana/`.
...
- Added `select` with full PYTHON_MODERN_STANDARD rule set (was missing entirely)
...
Changes from initial migration:
- Added `minversion`, `addopts`, `xfail_strict`, `filterwarnings` per standard
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/basana/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import basana" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv run python -c "import basana"           # Verify import
